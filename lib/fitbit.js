...
const secret = clientId + ':' + clientSecret;
    ...
    export const updateData = async (req, fitbit_id) => {
        ...
        const endDate = moment().utcOffset(-5, true).format('YYYY-MM-DD');
      ...
      db.collection("data").updateOne({"user.fitbit_id": fitbit_id}, {$set:distanceJson},{strict: false}, { upsert: false }, function(err, res) {
        if (err) throw err;
        console.log("1 document updated");
      });
       ...
       response = await fetch(`https://api.fitbit.com/1/user/-/activities/steps/date/${baseDate}/${endDate}.json`, {
        ...
        db.collection("data").updateOne({"user.fitbit_id": fitbit_id}, {$set:stepsJson}, {strict: false}, { upsert: false }, function(err, res) {
          if (err) throw err;
          console.log("1 document updated");
        });
        ...
        db.collection("data").updateOne({"user.fitbit_id": fitbit_id}, {$set:{recent: recent, lastUpdated: _.now()}},{strict: false}, { upsert: false }, function(err, res) {
          if (err) throw err;
          console.log("1 document updated");
        });
