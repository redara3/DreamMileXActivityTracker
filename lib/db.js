...
export async function getUser(req, fitbit_id) {
  console.log(fitbit_id);
    ...
    return {
      access_token: user.user.access_token, refresh_token: user.user.refresh_token, displayName: user.user.displayName, teamName: user.user.teamName, fitbit_id: user.user.fitbit_id
    }
  }

  export async function getActivity(req, fitbit_id) {
      ...
      _.forEach(activitydata, function(item) {
        ...
        returnarray.push({id, name, team, challenge, totalDistance, totalSteps})
      })
      return returnarray;
    }